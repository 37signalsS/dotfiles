...
```
git clone https://github.com/37signalsS/dotfiles.git
```
...
Чтобы удалить определённые приложения из Rofi или Wofi, удалите их `.desktop` файлы из:
...
Чтобы SwayNC не менял конфигурацию при обновлениях, создайте override-файл `~/.config/systemd/user/swaync.service.d/override.conf`:
...
```
sudo micro /etc/tlp.conf
```

Создайте службу Powertop:

```
sudo micro /etc/systemd/system/powertop.service
```
...
| Компонент       | Детали                                                              |
|-----------------|---------------------------------------------------------------------|
| Дистрибутив     | [CachyOS](https://cachyos.org/)                                     |
...
```bash
# Перейдите в директорию dotfiles
cd ~/dotfiles 

# Установите пакет nvim
stow nvim
```

Эта команда создаст символическую ссылку: `~/dotfiles/nvim/.config/nvim/init.lua` -> `~/.config/nvim/init.lua`.
...
Winapps позволяет запускать приложения Windows (например, Microsoft Office) в Linux, интегрируя их непосредственно в рабочий стол, используя RDP и виртуализацию в фоновом режиме.
...
```bash
mkdir -p ~/.config/winapps && cp compose.yaml ~/.config/winapps && docker compose --file ~/.config/winapps/compose.yaml up -d
```
